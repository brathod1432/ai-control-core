...
### **Scopes for AI assistant Skills**
...
- Adapt for ai-ops-skills context
...
if git diff --cached | grep -iE '(api[_-]?key|secret|password|token).*=.*[^x{5}]'; then
    echo "âŒ Potential secret detected in commit"
    exit 1
fi
...
if [ $? -ne 0 ]; then
    echo "âŒ Python syntax errors detected"
    exit 1
fi
...
if ! head -1 "$1" | grep -qE "$commit_regex"; then
    echo "âŒ Commit message does not follow conventional commits format"
...
if git diff origin/main...HEAD | grep -i "TODO"; then
    echo "âš ï¸  TODO comments found - consider addressing before push"
fi
...
- âœ… Use conventional commit format for all commits
- âœ… Reference issue numbers in commit messages
...
- âŒ Commit directly to main (use PRs)
- âŒ Force push to shared branches
- âŒ Commit secrets or credentials
- âŒ Mix multiple unrelated changes in one commit
- âŒ Use vague commit messages ("fix stuff", "updates")
- âŒ Skip quality checks
- âŒ Leave branches unmerged for extended periods
