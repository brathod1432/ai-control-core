...
LLM judges call the CLI tool the user is already running (AI assistant, Codex, Gemini). The evaluation prompt is locked inside `evaluate.py` â€” the agent cannot modify it. This prevents the agent from gaming its own evaluator.

The user's existing subscription covers the cost:
- an agent runtime Max â†’ unlimited AI assistant calls for evaluation
- an agent runtime (an AI chat client Pro) â†’ unlimited Codex calls
- an agent runtime (free tier) â†’ free evaluation calls
...
cp -r ai-ops-skills/engineering/autoresearch-agent ~/.claude/skills/
```
