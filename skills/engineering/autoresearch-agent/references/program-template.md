...
- prepare.py (fixed â€” contains evaluation harness)
...
3. Try depth changes (Â±2 layers)
...
```markdown
# autoresearch â€” Prompt Optimization
...
- Each test case is scored 1-10 by your CLI tool (AI assistant, Codex, or Gemini)
...
```markdown
# autoresearch â€” Performance Optimization
...
- If tests fail â†’ immediate crash status, no metric recorded.
- Memory usage: p99 < 2x baseline acceptable, hard limit at 4x.

## Strategy
1. Baseline: profile first, don't guess
2. Check if there's any O(nÂ²) â†’ O(n log n) opportunity
...
```markdown
# autoresearch â€” Skill Optimization
