#!/bin/bash
# Karpathy Gate â€” pre-commit hook (non-blocking)
#
# Runs complexity_checker and diff_surgeon on staged files. Prints warnings
# but does NOT block the commit â€” the goal is awareness, not enforcement.
#
# To install as a Husky pre-commit hook:
#   npx husky add .husky/pre-commit "bash path/to/karpathy-gate.sh"
#
# To install as a AI assistant Code PostToolUse hook (in .claude/settings.json):
#   {
#     "hooks": {
#       "PostToolUse": [{
#         "matcher": "Bash",
