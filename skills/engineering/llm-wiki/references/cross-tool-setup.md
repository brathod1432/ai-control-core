...
|---|---|---|
| an agent runtime | `AI_RUNTIME_GUIDE.md` | Loaded automatically when CC starts in the vault dir |
| an agent runtime (OpenAI) | `AGENTS.md` | Loaded at session start |
...
| an agent runtime | `AGENTS.md` | Same convention |
| Aider | `CONVENTIONS.md` or `.aider.conf.yml` | Point Aider at `AI_RUNTIME_GUIDE.md` with `--read AI_RUNTIME_GUIDE.md` |

**Recommendation:** ship **both** `AI_RUNTIME_GUIDE.md` and `AGENTS.md` in every vault. `init_vault.py --tool all` does this by default.
...
```bash
cd <vault>
ln -sf AI_RUNTIME_GUIDE.md AGENTS.md
...
- Clone ai-ops-skills and copy `engineering/llm-wiki/` into `~/.claude/skills/`, or
...
Same as Codex â€” drop `AGENTS.md` in the vault root and use natural language.

### Multi-tool same session

You can run an agent runtime and Codex **simultaneously** against the same vault. They'll both see updates if one writes a page â€” filesystem is the source of truth. Just make sure each vault is committed to git so you can resolve conflicts.
...
The wiki can be exposed as an MCP tool so any MCP-capable client (AI assistant Desktop, an agent runtime, etc.) can query it. See `engineering/mcp-design` in this repo for the pattern. A future version of this plugin will ship an `mcp/` directory with a reference MCP server.
