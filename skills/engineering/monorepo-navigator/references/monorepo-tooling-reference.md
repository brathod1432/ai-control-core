...
# Dry-run â€” see what would run without executing
...
```bash
# Using Turborepo â€” show affected packages
...
## an agent runtime Configuration (Workspace-Aware AI_RUNTIME_GUIDE.md)

Place a root AI_RUNTIME_GUIDE.md + per-package AI_RUNTIME_GUIDE.md files:

```markdown
# /AI_RUNTIME_GUIDE.md â€” Root (applies to all packages)

## Monorepo Structure
- apps/web       â€” Next.js customer-facing app
- apps/admin     â€” Next.js internal admin
- apps/api       â€” Express REST API
- packages/ui    â€” Shared React component library
- packages/utils â€” Shared utilities (pure functions only)
- packages/types â€” Shared TypeScript types (no runtime code)

## Build System
- pnpm workspaces + Turborepo
- Always use `pnpm --filter <package>` to scope commands
- Never run `npm install` or `yarn` â€” pnpm only
...
## Package Manager
pnpm â€” version pinned in packageManager field of root package.json
```

```markdown
# /packages/ui/AI_RUNTIME_GUIDE.md â€” Package-specific
...
- No direct API calls in components â€” accept data via props
- Every component needs a Storybook story in src/stories/
- Use Tailwind for styling â€” no CSS modules or styled-components
...
## Publishing
- Version bumps via changesets only â€” never edit package.json version manually
- Run `pnpm changeset` from repo root after changes
```

---

## Migration: Multi-Repo â†’ Monorepo
...
# apps/web/package.json: "@myorg/ui": "1.2.3" â†’ "@myorg/ui": "workspace:*"
...
### GitHub Actions â€” Affected Only
...
# In CI â€” version packages + update changelogs
