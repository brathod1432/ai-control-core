...
consistency, script --help, and referenced file existence â€” without
...
# â”€â”€ Check 1: JSON files â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def check_json(results):
    ...
    for path in json_files:
        ...
        if not os.path.exists(path):
            results.fail(f"{name} â€” file missing")
            continue
        try:
            with open(path) as f:
                data = json.load(f)
            results.ok(f"{name} â€” valid JSON")
        except json.JSONDecodeError as e:
            results.fail(f"{name} â€” invalid JSON: {e}")
            continue
        ...
        if name.endswith("plugin.json"):
            ...
            if extra:
                results.fail(f"{name} â€” disallowed fields: {extra}")
            else:
                results.ok(f"{name} â€” schema fields OK")
...
# â”€â”€ Check 2: YAML frontmatter â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def check_frontmatter(results):
    ...
    for path in skill_files:
        ...
        if not m:
            results.fail(f"{name} â€” missing YAML frontmatter")
            continue
        ...
        if missing:
            results.fail(f"{name} â€” frontmatter missing keys: {missing}")
        else:
            results.ok(f"{name} â€” frontmatter OK")


# â”€â”€ Check 3: Markdown structure â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def check_markdown(results):
    ...
    for path in md_files:
        ...
        if fence_count % 2 != 0:
            results.fail(f"{name} â€” unbalanced code fences ({fence_count} found)")
        else:
            results.ok(f"{name} â€” code fences balanced")
            ...
            if stripped.startswith("|") and stripped.endswith("|"):
                ...
                if not in_table:
                    ...
                    if not re.match(r"^\|[\s\-:|]+\|$", stripped):
                        results.warn(f"{name}:{i} â€” table column count mismatch ({pipes} vs {table_pipes})")
                        table_ok = False
            else:
                in_table = False
                table_pipes = 0
...
# â”€â”€ Check 4: Scripts --help â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
# â”€â”€ Check 5: Referenced files exist â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def check_references(results):
    ...
    for ref in expected:
        ...
        if os.path.exists(path):
            ...
            results.fail(f"{ref} â€” referenced but missing")


# â”€â”€ Check 6: Cross-domain coverage â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
def check_cross_domain(results):
    ...
    checks = [
        ...
        ("AI_RUNTIME_GUIDE.md", "content drafts"),
    ...
    for filepath, needle in checks:
        ...
        if not os.path.exists(path):
            results.fail(f"{filepath} â€” missing (cannot check cross-domain)")
            continue
        with open(path) as f:
            content = f.read()
        if needle.lower() in content.lower():
            results.ok(f"{filepath} â€” contains cross-domain example (\"{needle}\")")
        else:
            results.fail(f"{filepath} â€” missing cross-domain marker \"{needle}\"")
...
def main():
    ...
    if all_ok:
        ...
        print("\033[31mSome checks failed â€” see above.\033[0m")
        sys.exit(1)
