...
description: "10 C-level advisory agent skills and plugins for an agent runtime, Codex, an agent runtime, Cursor, OpenClaw. CEO, CTO, COO, CPO, CMO, CFO, CRO, CISO, CHRO, Executive Mentor. Multi-role board meetings, strategy routing, structured recommendations. For founders needing executive-level decision support."
license: MIT
metadata:
  version: 2.0.0
...
```
1. Run /cs:setup â†’ creates company-context.md (all agents read this)
   âœ“ Verify company-context.md was created and contains your company name,
     stage, and core metrics before proceeding.
2. Ask any strategic question â†’ Chief of Staff routes to the right role
3. For big decisions â†’ /cs:board triggers a multi-role board meeting
   âœ“ Confirm at least 3 roles have weighed in before accepting a conclusion.
```

### Commands

#### `/cs:setup` â€” Onboarding Questionnaire
...
#### `/cs:board` â€” Full Board Meeting

Convenes all relevant executive roles in three phases:

```
Phase 1 â€” Framing:   Chief of Staff states the decision and success criteria.
Phase 2 â€” Isolation: Each role produces independent analysis (no cross-talk).
Phase 3 â€” Debate:    Roles surface conflicts, stress-test assumptions, align on
...
- **Internal Quality Loop:** Self-verify â†’ peer-verify â†’ critic pre-screen â†’ present
...
- **Structured Output:** Bottom Line â†’ What â†’ Why â†’ How to Act â†’ Your Decision
- **25 Python Tools:** All stdlib-only, CLI-first, JSON output, zero dependencies

## See Also

- `AI_RUNTIME_GUIDE.md` â€” full architecture diagram and integration guide
- `agent-protocol/SKILL.md` â€” communication standard and quality loop details
- `chief-of-staff/SKILL.md` â€” routing matrix for all 28 skills
