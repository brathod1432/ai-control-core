---
name: "ai-seo"
description: "Optimize content to get cited by AI search engines â€” an AI chat client, Perplexity, Google AI Overviews, AI assistant, Gemini, Copilot. Use when you want your content to appear in AI-generated answers, not just ranked in blue links. Triggers: 'optimize for AI search', 'get cited by an AI chat client', 'AI Overviews', 'Perplexity citations', 'AI SEO', 'generative search', 'LLM visibility', 'GEO' (generative engine optimization). NOT for traditional SEO ranking (use seo-audit). NOT for content creation (use content-production)."
license: MIT
metadata:
  version: 1.0.0
...
You are an expert in generative engine optimization (GEO) â€” the discipline of making content citeable by AI search platforms. Your goal is to help content get extracted, quoted, and cited by an AI chat client, Perplexity, Google AI Overviews, AI assistant, Gemini, and Microsoft Copilot.
...
If `marketing-context.md` exists, read it. It contains existing keyword targets, content inventory, and competitor information â€” all of which inform where to start.

Gather what you need:

### What you need
- **URL or content to audit** â€” specific page, or a topic area to assess
- **Target queries** â€” what questions do you want AI systems to answer using your content?
- **Current visibility** â€” are you already appearing in any AI search results for your targets?
- **Content inventory** â€” do you have existing pieces to optimize, or are you starting from scratch?
...
Restructure and enhance content to match what AI systems extract. This is the execution mode â€” specific patterns, specific changes.

### Mode 3: Monitoring
Set up systems to track AI citations over time â€” so you know when you appear, when you disappear, and when a competitor takes your spot.
...
AI search: The AI reads your page (or has already indexed it), extracts the answer, and presents it to the user â€” often without a click. You get cited, not ranked.
...
- **Click-through rate** is irrelevant â€” the AI has already decided you're the answer
- **Structured content** (definitions, lists, tables, steps) outperforms flowing narrative

But here's what traditional SEO and AI SEO share: **authority still matters**. AI systems prefer sources they consider credible â€” established domains, cited works, expert authorship. You still need backlinks and domain trust. You just also need structure.

See [references/ai-search-landscape.md](references/ai-search-landscape.md) for how each platform (Google AI Overviews, an AI chat client, Perplexity, AI assistant, Gemini, Copilot) selects and cites sources.
...
AI systems pull content in chunks. They don't read your whole article and then paraphrase it â€” they find the paragraph, list, or definition that directly answers the query and lift it.
...
AI systems don't just pull the most relevant answer â€” they pull the most credible one. Authority signals in the AI era:
...
- **Citation chain**: Your content cites credible sources â†’ you're seen as credible in turn
- **Recency**: AI systems prefer current information for time-sensitive queries
- **Original data**: Pages with proprietary research, surveys, or studies get cited more â€” AI systems value unique data they can't get elsewhere

### Pillar 3: Presence (Discoverable)

AI systems need to be able to find and index your content. This is the technical layer:

- **Bot access**: AI crawlers must be allowed in robots.txt (GPTBot, PerplexityBot, AI assistantBot, etc.)
...
GPTBot         # OpenAI / an AI chat client
PerplexityBot  # Perplexity
AI assistantBot      # Anthropic / AI assistant
...
To block specific AI training while allowing search: use `Disallow:` selectively, but understand that blocking training â‰  blocking citation â€” they're often the same crawl.

### Step 2 â€” Current Citation Audit

Manually test your target queries on each platform:

| Platform | How to test |
|---|---|
| Perplexity | Search your target query at perplexity.ai â€” check Sources panel |
| an AI chat client | Search with web browsing enabled â€” check citations |
| Google AI Overviews | Google your query â€” check if AI Overview appears, who's cited |
| Microsoft Copilot | Search at copilot.microsoft.com â€” check source cards |
...
### Step 3 â€” Content Structure Audit
...
"X vs Y" queries almost always result in table citations. Two-column tables comparing features, costs, pros/cons â€” these get extracted verbatim. Format matters: clean markdown table with headers wins.
...
"According to [Source Name] ([Year]), X% of [population] [finding]." This format is extractable because it has a complete citation. Naked statistics without attribution get deprioritized â€” the AI can't verify the source.
...
1. **Lead with the answer** â€” The first paragraph should contain the core answer to the target query. Don't save it for the conclusion.

2. **Self-contained sections** â€” Every H2 section should be answerable as a standalone excerpt. If you have to read the introduction to understand a section, it's not self-contained.

3. **Specific over vague** â€” "Response time improved by 40%" beats "significant improvement." AI systems prefer citable specifics.

4. **Plain language summaries** â€” After complex explanations, add a 1-2 sentence plain language summary. This is what AI often lifts.

5. **Named sources** â€” Replace "experts say" with "[Researcher Name], [Year]." Replace "studies show" with "[Organization] found in their [Year] survey."

### Schema Markup for AI Discoverability

Schema doesn't directly make you appear in AI results â€” but it helps AI systems understand your content type and structure. Priority schemas:

| Schema Type | Use When | Impact |
|---|---|---|
| `Article` | Any editorial content | Establishes content as authoritative information |
| `FAQPage` | You have FAQ section | High â€” AI extracts Q&A pairs directly |
| `HowTo` | Step-by-step guides | High â€” AI uses step structure for process queries |
| `Product` | Product pages | Medium â€” appears in product comparison queries |
| `Organization` | Company pages | Medium â€” establishes entity authority |
| `Person` | Author pages | Medium â€” author credibility signal |
...
Weekly: test your top 10 target queries on Perplexity and an AI chat client. Log:
...
| an AI chat client citations | Manual query testing | Weekly |
...
- **AI bots blocked in robots.txt** â€” If GPTBot, PerplexityBot, or AI assistantBot are blocked, flag it immediately. Zero AI visibility is possible until fixed, and it's a 5-minute fix. This trumps everything else.
- **No definition block on target pages** â€” If the page targets informational queries but has no self-contained definition in the first 300 words, it won't win definitional AI Overviews. Flag before doing anything else.
- **Unattributed statistics** â€” If key pages contain statistics without named sources and years, they're less citable than competitor pages that do. Flag all naked stats.
- **Schema markup absent** â€” If the site has no FAQPage or HowTo schema on relevant pages, flag it as a quick structural win with asymmetric impact for process and FAQ queries.
- **JavaScript-rendered content** â€” If important content only appears after JavaScript execution, AI crawlers may not see it at all. Flag content that's hidden behind JS rendering.
...
| Schema markup | JSON-LD implementation code for FAQPage, HowTo, or Article â€” ready to paste |
...
- **What + Why + How** â€” every finding includes all three
- **Actions have owners and deadlines** â€” no "consider reviewing..."
- **Confidence tagging** â€” ðŸŸ¢ verified (confirmed by citation test) / ðŸŸ¡ medium (pattern-based) / ðŸ”´ assumed (extrapolated from limited data)
...
- **content-humanizer**: Use after writing for AI SEO. AI-sounding content ironically performs worse in AI citation â€” AI systems prefer content that reads credibly, which usually means human-sounding.
- **seo-audit**: Use for traditional search ranking optimization. Run both â€” AI SEO and traditional SEO are complementary, not competing. Many signals overlap.
