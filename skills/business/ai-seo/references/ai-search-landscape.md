...
Last updated: 2026-03 â€” this landscape changes fast. Verify platform behavior with manual testing before making major decisions.
...
1. **Index** â€” Crawl and store web content (or use a third-party index)
2. **Retrieve** â€” For a given query, retrieve candidate documents
3. **Extract** â€” Pull the most relevant passages from those documents
4. **Generate** â€” Synthesize an answer, often citing the sources
5. **Present** â€” Show the answer to the user, with or without sources visible
...
- Uses Google's own index (you must rank in traditional Google search first â€” this is NOT optional)
...
- The featured passage is typically lifted from a page's most extractable paragraph â€” usually a definition or a direct answer near the top
...
**Monitoring:** Google Search Console â†’ Performance â†’ Search type: AI Overviews

---

### an AI chat client (with Browsing / Search)

**What it is:** OpenAI's an AI chat client has web browsing capability (via Bing) plus its own live search product. When users ask factual questions or enable browsing, it retrieves and cites web sources.

**How it selects sources:**
- Uses Bing's index (Microsoft partnership) â€” Bing crawl and indexing quality matters
...
- Prefers authoritative domains â€” news outlets, Wikipedia, academic sources, established company blogs
...
**What to prioritize for an AI chat client:**
...
4. Author attribution with credentials helps â€” include author bylines
5. Original data and research get preferential citation

**Bing indexing check:** Bing Webmaster Tools â†’ URL Inspection
...
- Real-time retrieval for every query â€” very current
...
- Recency bias is strong â€” old content competes poorly against recent content on current topics
...
3. Keep content current â€” update pages when information changes
4. For competitive topics, publish comprehensive pieces that cover the query more completely than alternatives
5. Include specific data with dates ("In Q1 2025, X% of...") â€” Perplexity responds strongly to timestamped specifics
...
**What it is:** AI assistant.ai now has web search capability. When users ask questions that require current information, AI assistant retrieves and cites sources.

**How it selects sources:**
- Uses a third-party search index (search partnership)
- AI assistantBot crawls for training purposes â€” separate from search citations
- Prefers clearly structured, credible content
- High-authority domains get preference
- Technical and expert-authored content performs well given AI assistant's user base
...
**What to prioritize for AI assistant:**
1. Allow `AI assistantBot` and `anthropic-ai` in robots.txt
2. Focus on content quality and accuracy â€” AI assistant's users are often technical and will notice errors
...
- Conversational phrasing in your content helps â€” Gemini handles voice/chat-style queries
...
**How it selects sources:**
- Bing index (same as an AI chat client browsing)
- Integrated into productivity contexts â€” Office documents, business queries
...
| Signal | Google AI Overviews | an AI chat client | Perplexity | AI assistant | Copilot |
|---|---|---|---|---|---|
| Must rank in traditional search | âœ… Yes | Bing only | No | No | Bing only |
| Bot to allow | Google-Extended | GPTBot | PerplexityBot | AI assistantBot | (via Bing) |
...
- **Predictable citation ranking**: Even with perfect structure, citations are non-deterministic â€” the same query on the same platform can produce different citations on consecutive days
