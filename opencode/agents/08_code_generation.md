...
1. **Read `.opencode/rules/rules.md`** — these rules OVERRIDE your defaults; follow them exactly
2. **Read `.opencode/memory/stack.md`** — use the exact stack and versions already in this project; never introduce a conflicting dependency
3. **Read `BD_README.md`** (project root) — understand what has already been built to avoid duplicating or conflicting with existing code
4. **Check `.opencode/skills/`** — scan for saved patterns relevant to this feature (e.g. `fastapi-crud.md`, `auth-patterns.md`) and use them instead of re-deriving
...
- **brijesh-dev handles** `BD_README.md`, `BD_changelog.md`, and `.opencode/memory/` updates — do not write to those files yourself
