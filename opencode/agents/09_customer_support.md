...
2. You may read:
   - `BD_README.md` (project root) — product name, current phase (MVP/Beta/Production), and known issues so your reply is accurate
...
Return your classification + draft reply JSON to brijesh-dev. It logs the output in `BD_changelog.md`. If a recurring issue pattern is found across multiple emails, flag it so brijesh-dev can save it as a known issue in `BD_README.md` and `.opencode/memory/context.md`.
