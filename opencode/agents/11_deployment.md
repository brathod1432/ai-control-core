...
1. **Read `.opencode/memory/stack.md`** — use the exact infra, hosting, and Python version already confirmed for this project; never assume defaults that contradict it
2. **Read `BD_README.md`** (project root) — check the current deploy status; if already deployed, produce an update/re-deploy plan rather than a fresh deploy plan
3. **Read `.opencode/rules/rules.md`** — check for deploy-specific constraints (e.g. "no paid hosting", "must use GitHub Actions")
4. **Check `.opencode/skills/deploy-<target>.md`** — if a prior deploy was documented, reuse its env var list and checklist as a starting point
...
- **brijesh-dev handles** `BD_README.md`, `BD_changelog.md`, and `.opencode/memory/stack.md` updates — do not write to those files yourself
