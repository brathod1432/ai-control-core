...
2. You may read:
   - `BD_README.md` (project root) — check if this project already has prior tasks so you don't re-plan what's done
   - `.opencode/memory/context.md` — project constraints that shape the task list (e.g. known tech restrictions)
   - `BD_todo.md` (project root) — existing backlog items so you don't duplicate them

Return your task list JSON to brijesh-dev. It writes the task file at `BD_tasks/<task-id>.json` and handles all memory updates.
