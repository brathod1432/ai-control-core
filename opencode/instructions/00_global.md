...
## Project Memory Convention (BD_* Pattern)

Every project uses this structure at its root:

```
<project_dir>/
├── BD_README.md       ← READ THIS FIRST in any project session
├── BD_changelog.md    ← append-only task history
├── BD_todo.md         ← project backlog
├── BD_tasks/          ← task JSON files (bdev-YYYYMMDD-NNN.json)
...
**Always read `BD_README.md` before starting any work in a project.**
...
```jsonc
"instructions": [
  "C:/Users/kbrat/.config/opencode/instructions/00_global.md",
  "C:/Users/kbrat/.config/opencode/instructions/01_coding.md",
  "C:/Users/kbrat/.config/opencode/instructions/02_workflow.md",
  "C:/Users/kbrat/.config/opencode/instructions/skills/**/*.md"
]
```
...
| File | Purpose |
|---|---|
| `00_global.md` | Identity, provider, BD_* convention, agent roster, this skills guide |
...
| `skills/fastapi_crud.md` | Full CRUD scaffold: models, schemas, crud, router, tests |
| `skills/nvidia_api.md` | NVIDIA API client patterns, model list, streaming, errors |

### Adding a New Global Skill

1. Create a new `.md` file under `instructions/skills/`:
   ```
   C:\Users\kbrat\.config\opencode\instructions\skills\<topic>.md
   ```
...
3. No changes to `opencode.jsonc` needed — the glob `skills/**/*.md` picks it up automatically.
