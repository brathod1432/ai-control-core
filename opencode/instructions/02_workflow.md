...
1. **Check for `BD_README.md`** at the project root
   - If found → read it completely before touching anything else
   - If missing → ask "Is this a new project? Should I initialise BD_* files?"
2. **Check for `.opencode/rules/rules.md`** — read it; it overrides global rules
3. **Check `BD_todo.md`** — note any open backlog items relevant to the request
...
For quick single-file tasks, still log a one-line entry to `BD_changelog.md`.
...
| FINAL SUMMARY | BD_README.md updated; BD_changelog.md appended |
...
Every task — even tiny ones — appends to `BD_changelog.md`:
...
- When creating new files, also update `BD_todo.md` if follow-up tasks exist
